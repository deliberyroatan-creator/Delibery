  ...
  const [loading, setLoading] = useState(true);
    ...
    useEffect(() => {
      ...
      const restMatches = restaurantes
        ...
        .slice(0, 4)
        .map((r) => ({ type: "restaurante", id: r.id, nombre: r.nombre, descripcion: r.descripcion, emoji: r.emoji }));

      const platMatches = platillos
        .filter((p) => `${p.nombre} ${p.descripcion}`.toLowerCase().includes(q))
        .slice(0, 6 - restMatches.length)
        .map((p) => ({ type: "platillo", id: p.id, nombre: p.nombre, descripcion: p.descripcion, restaurante: p.restaurante }));
      ...
      return (
        <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "#ffffff" }}>
          <div style={{ textAlign: "center" }}>
            ...
            <p style={{ color: "#6b7280", fontSize: 18 }}>Cargando restaurantes...</p>
          </div>
        </div>
      );
    }
    ...
    return (
      <div style={{ background: "#ffffff", minHeight: "100vh", paddingBottom: "80px", position: "relative" }}>
        <Header />
        ...
        <main style={{ maxWidth: "500px", margin: "0 auto", padding: "16px" }}>
          ...
          <div style={{ marginTop: 12, marginBottom: 18, position: "relative" }}>
            <input
              ...
              style={{ width: "100%", padding: "12px 14px", borderRadius: 12, border: "1px solid #e6e7eb", outline: "none", fontSize: 14 }}
            />

            {showSuggestions && search.trim().length > 0 && suggestions.length > 0 && (
              <div style={{ position: "absolute", top: "calc(100% + 8px)", left: 0, right: 0, background: "rgba(255,255,255,0.98)", border: "1px solid #e6e7eb", borderRadius: 12, boxShadow: "0 8px 24px rgba(2,6,23,0.06)", zIndex: 40, maxHeight: 260, overflow: "auto" }}>
                {suggestions.map((s, idx) => (
                  <div key={idx} onMouseDown={() => { if (s.type === "restaurante") navigate(`/restaurante/${s.id}`); else if (s.type === "platillo") navigate(`/platillo/${s.id}`); }} style={{ padding: "10px 12px", display: "flex", flexDirection: "column", gap: 4, cursor: "pointer", borderBottom: idx < suggestions.length - 1 ? "1px solid #f3f4f6" : "none" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ fontWeight: 700, color: "#0f172a" }}>{s.nombre}</div>
                      <div style={{ fontSize: 13, color: "#6b7280" }}>{s.type === "restaurante" ? "Restaurante" : "Platillo"}</div>
                    </div>
                    {s.descripcion && <div style={{ fontSize: 13, color: "#6b7280" }}>{s.descripcion}</div>}
                  </div>
                ))}
              </div>
            )}
          ...
          <PlatillosCarousel platillos={platillos.sort(() => Math.random() - 0.5)} />

          <section>
            <h2 style={{ fontSize: "24px", fontWeight: 800, color: "#111827", marginBottom: "20px", display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "14px" }}>
              {categorias.length > 0 ? (
                categorias.map((categoria) => (
                  <div key={categoria.id} onClick={() => navigate(`/categoria/${categoria.id}`)} style={{ background: `linear-gradient(135deg, ${categoria.color_gradiente_inicio} 0%, ${categoria.color_gradiente_fin} 100%)`, borderRadius: "16px", padding: "20px 16px", position: "relative", overflow: "hidden", cursor: "pointer", transition: "all 0.3s ease", boxShadow: "0 6px 18px rgba(102,126,234,0.3)", minHeight: "140px", clipPath: "ellipse(65% 60% at 30% 35%)", WebkitClipPath: "ellipse(65% 60% at 30% 35%)" }} onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.transform = "translateY(-4px)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 8px 24px rgba(102,126,234,0.4)"; }} onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.transform = "translateY(0)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 6px 18px rgba(102,126,234,0.3)"; }}>
                    <div style={{ position: "absolute", top: "-15px", right: "-15px", fontSize: "90px", opacity: 0.15 }}>{categoria.emoji}</div>
                    <div style={{ position: "relative", zIndex: 2 }}>
                      <div style={{ fontSize: "40px", marginBottom: "8px" }}>{categoria.emoji}</div>
                      <h3 style={{ color: "#fff", fontSize: "17px", fontWeight: 700, margin: "0 0 6px 0" }}>{categoria.nombre}</h3>
                    </div>
                  </div>
                ))
              ) : (
                <div style={{ background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", borderRadius: "16px", padding: "20px 16px", position: "relative", overflow: "hidden", cursor: "pointer", minHeight: "140px", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", clipPath: "ellipse(65% 60% at 30% 35%)", WebkitClipPath: "ellipse(65% 60% at 30% 35%)" }}>Cargando...</div>
              )}
            </div>
          </section>
        </main>
